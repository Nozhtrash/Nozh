...
public final class BenchmarkSuite {
    ...
    private void finishBenchmark(long elapsedMs) {
        ...
        NozhConstants.LOGGER.info("   Avg FPS: {:.2f}", avgFps);
        NozhConstants.LOGGER.info("   Min FPS: {:.2f}", minFps);
        NozhConstants.LOGGER.info("   Max FPS: {:.2f}", maxFps);
        ...
        HardwareBenchmarker.getInstance().runBenchmarkAsync().thenAccept(score -> {
            NozhConstants.LOGGER.info("   Hardware Score: {:.2f} (CPU)", score);
        });
    }
