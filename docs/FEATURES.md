...
### 🧙‍♂️ First Run Wizard
**For Dummies**: A popup that asks "Is your PC fast or slow?" so you don't have to guess settings.
**For Experts**: A logic flow that pre-seeds the `OptimizationProfile` based on user selection, applying a preset of `ActionMatrixRules` immediately to prevent initial world-load stutters.
